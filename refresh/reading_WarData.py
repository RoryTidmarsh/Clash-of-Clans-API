...
class member:
    def __init__(self, member_json):
        ...
        self.attacker_townhallLevel = (
            np.nan
        )  # TH level of the opponent that is attacking us
        ...
        self.defense_th_diff = (
            np.nan
        )  # TH diff compared to the opponent that attacked us
        ...
        if "attacks" not in member_json or member_json["attacks"] is None:
            ...
            if debug_print_statements:
                print(f"Member '{member_json['name']}' has not attacked.")
            ...
            self.defender_tag = member_json["attacks"][0][
                "defenderTag"
            ]  # Tag of person we attack

        if (
            "bestOpponentAttack" not in member_json
            or member_json["bestOpponentAttack"] is None
        ):
            ...
            if debug_print_statements:
                print(f"Member '{member_json['name']}' has not been attacked.")

        else:
            self.defense_stars = member_json["bestOpponentAttack"]["stars"]
            self.defense_percentage = member_json["bestOpponentAttack"][
                "destructionPercentage"
            ]
            self.defense_duration = member_json["bestOpponentAttack"]["duration"]
            self.attacker_tag = member_json["bestOpponentAttack"][
                "attackerTag"
            ]  # Opponent that attacks our clan

    def __repr__(self):
        if not np.isnan(self.attack_stars):
            th_diff = (
                self.attack_th_diff
                if not np.isnan(self.defender_townhallLevel)
                else "?"
            )
        ...
        if not np.isnan(self.defense_stars):
            th_diff = (
                self.defense_th_diff
                if not np.isnan(self.attacker_townhallLevel)
                else "?"
            )
            th_info = f" (TH{th_diff:+d})" if th_diff != "?" else ""
        ...
        return (
            f"Member({self.name}, tag={self.tag}, TH{self.townhallLevel}, pos={self.mapPosistion})\n"
    ...
    def find_attacker_TH_level(self, opponent_member_json):
        ...
        self.attack_th_diff = (
            self.defender_townhallLevel - self.townhallLevel
            if not np.isnan(self.defender_townhallLevel)
            else np.nan
        )
        self.defense_th_diff = (
            self.attacker_townhallLevel - self.townhallLevel
            if not np.isnan(self.attacker_townhallLevel)
            else np.nan
        )
        ...
        if debug_print_statements:
            print(
                f"attacker TH level {self.attacker_townhallLevel}, defender TH level {self.defender_townhallLevel}"
            )
    ...
    def to_dataframe_row(self):
        ...
        return {
            ...
            "attack_th_diff": self._to_int(
                self.attack_th_diff
            ),  # Positive means attacked higher TH
            "defense_th_diff": self._to_int(
                self.defense_th_diff
            ),  # Positive means defended against higher TH
    ...
    def _to_int(self, value):
        ...
        try:
            ...
            print(
                f"Warning: Could not convert value '{value}' of type '{type(value)}' to int."
            )
            return None
...
def get_war_stats(battle_tag):
    ...
    if debug_print_statements:
        print("Pussay war link: ", war_link)
    ...
    if war_response.status_code != 200:
        ...
        raise ValueError(
            f"API request failed with status code {war_response.status_code}: {error_msg}"
        )
    ...
    if "state" in war_data:
        ...
        raise ValueError(
            f"Clan War not accessible. War occurred too long ago or data is unavailable. War state: {war_data["state"]}"
        )
    ...
    Pussay_clan_or_opponent = (
        "clan" if war_data["clan"]["name"] == clan_name else "opponent"
    )
    Opponent_clan_or_opponent = (
        "opponent" if Pussay_clan_or_opponent == "clan" else "clan"
    )
    if debug_print_statements:
...
class WarDataManager:
    ...
    def get_war_status(self, wartag):
        ...
        """
        war_status = (
            supabase.table(self.status_table)
            .select("*")
            .eq("wartag", wartag)
            .execute()
            .data
    ...
    def should_load_war(self, wartag):
        ...
        if loading_status in ["completed", "Error - too old"]:
            print(
                f"War {wartag} already marked as '{loading_status}' (COC status: {coc_status}). Skipping."
            )
            return False

        # Reload if 'notLoaded' or 'inProgress'
        print(
            f"War {wartag} loading status: {loading_status}, COC status: {coc_status}. Will reload."
        )
        return True
        ...
        for key, dtype in schema.items():
            ...
            if dtype is int:
                try:
                    ...
                    if val is None:
                        cleaned[key] = None
                    elif isinstance(val, float) and (
                        np.isnan(val) or str(val).lower() == "nan"
                    ):
                        cleaned[key] = None
    ...
    def save_war_data(self, wartag, war_df, coc_war_status, season, battleday):
        ...
        if war_df is not None and not war_df.empty:
            ...
            for record in records:
                ...
                try:
                    ...
                    if not exists:
                        print(
                            f"Inserted new war data for {record['tag']} in war {record['wartag']}"
                        )
                        # Insert new row
                        ...
                        print(
                            f"Updated war data for {record['tag']} in war {record['wartag']}"
                        )
                        # Update existing row
                        response = (
                            supabase.table("war_data")
                        ...
                        if response.data is None:
                            raise ValueError(
                                f"Supabase response data is None for war data upsert of {record['tag']} in war {record['wartag']}."
                            )
                    # else:
        ...
        try:
            ...
            status_query = (
                supabase.table("war_status").select("id").eq("wartag", wartag).execute()
            )
            ...
            if not status_exists:
                ...
                supabase.table("war_status").update(war_status_record).eq(
                    "wartag", wartag
                ).execute()

            status_symbol = "✓" if loading_status == "completed" else "⋯"
            if coc_war_status == "notInWar" or loading_status in [
                "Error - too old",
                "notLoaded",
            ]:
                print(
                    f"✗ Skipping war {wartag} | Marked as {coc_war_status} ({loading_status}) — will not reload again."
                )
            else:
                print(
                    f"{status_symbol} Saved war {wartag} | COC war status: {coc_war_status} | Loading: {loading_status}"
                )
    ...
    def load_cached_war_data(self, wartag):
        ...
        war_data = (
            supabase.table("war_data").select("*").eq("wartag", wartag).execute().data
        )
        if war_data:
    ...
    def process_war(self, wartag, get_war_stats_func, season=None, battleday=None):
        ...
        if not self.should_load_war(wartag):
            ...
            war_status = self.get_war_status(wartag)
            print(
                f"⚠ No cached data for {wartag} — but it's marked as '{war_status['loading_status']}'. Skipping API call."
            )
            return (
                pd.DataFrame(),
                war_status["coc_war_status"],
        ...
        try:
            ...
            self.save_war_data(
                wartag, war_df, coc_war_status, season=season, battleday=battleday
            )
    ...
    def get_status_summary(self):
        ...
        if status_df.empty:
            return {"total_wars": 0, "completed": 0, "in_progress": 0, "not_loaded": 0}
        ...
        return {
            ...
            "not_loaded": len(
                status_df[status_df["loading_status"].isin(["notLoaded"])]
            ),
...
def load_warData_supabase():
    ...
    for idx, row in Pussay_wars_df.iterrows():
        ...
        if wartag == "#0":
            print(
                f"○ Skipping placeholder war tag for Season {season}, Battle Day {battleday}"
            )
            continue
        ...
        try:
            ...
            elif coc_status == "notInWar" or loading_status in [
                "Error - too old",
                "notLoaded",
            ]:
                pass
